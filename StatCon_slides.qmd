...
| [weightedCVA]{style="font-family: 'Noto Sans Mono', monospace"} | The default is "weighted", specifying a weighted CVA to be performed. Other possible values are "unweightedI" and "unweightedCent". *Only weighted in current version*. |
...
## [CVA()]{style="font-family: 'Noto Sans Mono', monospace"} with [plot()]{style="font-family: 'Noto Sans Mono', monospace"} - <font size="8">weighted CVA</font>

```{r, echo = TRUE, eval = TRUE}
biplot(Diabetes[,3:5], group.aes = Diabetes[,6]) |> CVA(weightedCVA = "weighted") |>  axes(tick.label.pos = 
  c(1,2,1,4), tick.label.offset = c(0.7,0.2,0.4,1), label.dist = 0.2) |> plot()
```
