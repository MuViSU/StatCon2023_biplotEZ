...
```{r}
library(heplots)
biplot(Diabetes[,1:5],group.aes = Diabetes$group) |> CVA() |> plot()
```
...
| [dim.biplot]{style="font-family: 'Noto Sans Mono', monospace"} | Dimension of the biplot. Only values 1, 2 and 3 are accepted, with default 2.  *Only 2D in current version*.  |
| [e.vects]{style="font-family: 'Noto Sans Mono', monospace"}    | Which eigenvectors (principal components) to extract, with default 1:dim.biplot. |
...
| [weightedCVA]{style="font-family: 'Noto Sans Mono', monospace"} | The default is "weighted", specifying a weighted CVA to be performed. Other possible values are "unweightedI" and "unweightedCent". |
